CREATE TABLE IF NOT EXISTS `macws_db`.`projects_types` (
...
CREATE TABLE IF NOT EXISTS `macws_db`.`projects` (
  ...
  INDEX `fk_projects_feedback_types1_idx` (`feedback_types_id` ASC)  COMMENT '',
  CONSTRAINT `fk_projects_users1`
    FOREIGN KEY (`creator`)
    REFERENCES `macws_db`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_projects_feedback_types1`
    FOREIGN KEY (`feedback_types_id`)
    REFERENCES `macws_db`.`projects_types` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
